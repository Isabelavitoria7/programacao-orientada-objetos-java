...
public class App {
	...
	public static void main(String[] args) {
		...
		do {
			...
			String str_num_conta;
			Integer int_num_conta;
			...
			switch (escolha) {
				...
				case 0: {
					...
					for (int i = 0; i < vetContaC.length; i++) {
						if (vetContaC[i] == null) {
							str_num_conta = JOptionPane.showInputDialog("Digite o numero da conta a ser criada");
							int_num_conta = Integer.parseInt(str_num_conta);
							String senha = JOptionPane.showInputDialog("Digite a senha da conta a ser criada");

							vetContaC[i] = new ContaCorrente(senha, int_num_conta, 0, 0, 0);
				...
				case 1: {
					...
					for (int i = 0; i < vetContaP.length; i++) {
						if (vetContaP[i] == null) {
							...
							vetContaP[i] = new ContaPoupanca(senha, int_num_conta, 0, Taxa);
				...
				case 2: {
					str_num_conta = JOptionPane.showInputDialog("Digite o numero da conta");
					int_num_conta = Integer.parseInt(str_num_conta);
					...
					for (int i = 0; i <= vetContaC.length; i++) {
						if (vetContaC[i].getNumero() == int_num_conta
								&& vetContaC[i].getSenha().equalsIgnoreCase(senha)) {
				...
				case 3: {
					...
					for (int i = 0; i < vetContaP.length; i++) {
						if (vetContaP[i].getNumero() == NumeroConta
								&& vetContaP[i].getSenha().equalsIgnoreCase(senha)) {
				...
				case 8: {
					...
					Integer NumeroConta = Integer.parseInt(numeroConta);
					String senha = JOptionPane.showInputDialog("Digite a senha da conta");
					String atualizacheque = JOptionPane
							.showInputDialog("Digite o novo valor do cheque especial da conta");
					float NovoChequeEspecial = Float.parseFloat(atualizacheque);
					for (int i = 0; i < vetContaC.length; i++) {
						if (vetContaC[i].getNumero() == NumeroConta
								&& vetContaC[i].getSenha().equalsIgnoreCase(senha)) {
							vetContaC[i].setChequeEspecial(NovoChequeEspecial);
							JOptionPane.showMessageDialog(null, "Valor do cheque especial foi atualizado para: "
									+ vetContaC[i].getChequeEspecial());
							vetContaC[i].AtualizaTransacao();
							break;
						}
					}
